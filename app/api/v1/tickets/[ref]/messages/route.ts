 ...
 * "API: <key name>", and the owner is mailed about it — as when staff answer
 * on the site — unless "notify": false.
 */
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ ref: string }> }) {
  try {
    ...
    const ticket = await prisma.ticket.findUnique({ where: refWhere(ref), include: { user: { select: { name: true, email: true } } } })
    ...
    if (parsed.notify && owner) {
      ...
      const mail = sendMail({ to: owner, subject: subjects.newMessageUser(ticket, ticket.subject), html: mailNewMessageToUser(info, parsed.content, authorName) })
      after(async () => { await mail })
    }
